...
KIOKU **acumula automáticamente tus conversaciones en una Wiki** y **las recupera en la siguiente sesión**.
...
1. Crea un nuevo Vault en Obsidian (por ejemplo, `~/kioku-vault/main`)
2. Crea un repositorio privado en GitHub (por ejemplo, `kioku-vault`)
3. En el directorio del Vault: `git init && git remote add origin ...` (o `gh repo create --private --source=. --push`)

Este paso no está automatizado por los scripts de KIOKU. La autenticación con GitHub (gh CLI / claves SSH) depende de tu entorno.
...
# Abre ~/kioku-vault/main/ como Vault en Obsidian
...
|---|---|---|
| `OBSIDIAN_VAULT` | ninguno (requerido) | Raíz del Vault. auto-ingest/lint recurren a `${HOME}/kioku-vault/main` como alternativa |
...
- **`kioku_health` (11° MCP tool)** — Retorna 6 metricas core como JSON: `orphan` / `stale (>30d)` / `duplicate title` / `hot.md age` / `last ingest` / `unprocessed session-logs`. Cada metrica con `next_actions` concreto. Read-only.
- **`scripts/generate-health.mjs`** — `bash scripts/generate-health.mjs` escribe `wiki/meta/health.md`. Dashboard view "Wiki Health" agregado a `templates/wiki/meta/dashboard.base`.
- **Sprint 2 stretch (planeado para v0.7.5)** — broken wikilink / source_sha256 duplicate / etc.
- Tests: **23 BLUE-HEALTH-* + MCP-HEALTH-* + 9 drift (11 tools) + 475+ Node + 22 Bash all green**
...
v0.7.3 marca la **finalizacion del Sprint 1 reliability roadmap** (4 PRs en 8 dias). El delta de v0.7.2 es Mode A/B/C onboarding gradient + `doctor` mode detection.

- **Mode A/B/C onboarding (#4)** — Quick Start ofrece 3 install paths:
  - **Mode A: MCP-only** — Claude Desktop / security-conscious / casual try
  - **Mode B: Read-only** — *planned for v0.7.x*, enterprise / legal review
  - **Mode C: Full memory** — existing Manual Setup renamed (zero regression)
- **`doctor` mode detection** — `[mode]` line + `--json install_mode` field. 8 BLUE-DOCTOR-MODE-* tests (44 total)
- **Sprint 1 completion** — 4 PRs landed: #84 doctor + #89 drift test + #90 URL test stabilization + #96 Mode A/B/C
- Tests: **Node 475 + Bash + 44 doctor + 9 drift all green**
...
- Servidor MCP local stdio (`mcp/`) que expone seis herramientas — `kioku_search`, `kioku_read`, `kioku_list`, `kioku_write_note`, `kioku_write_wiki`, `kioku_delete`
