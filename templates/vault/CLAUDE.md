...
## hot.md (ホットキャッシュ、v0.5.1 Phase B)

`wiki/hot.md` は直近セッションの短い引き継ぎメモ (Recent Context) です。
以下 2 経路で自動注入されます:

- **SessionStart**: index.md と一緒に注入 (存在すれば)
- **PostCompact**: context compaction 後に hot.md のみ再注入 (LLM context を引き戻す)

### hot.md の更新ルール

- **書き出し頻度**: 単セッション内で状況が大きく変わったとき、または複数セッションを跨ぐ作業の引き継ぎが必要なときのみ
- **サイズ上限**: 500 word 以内 (soft) / 4000 char (hard truncate、超過は `... (truncated)` 付加)
- **書き方**: 「今何をやっているか / 次に何をすべきか / 引き継ぎで必要な context」の 3 点を簡潔に
- **書き込み方法**: `kioku_write_wiki` MCP tool、または手動で Edit
- **禁止**: 秘密情報 (API key / token / PII) は絶対に含めない (`scan-secrets.sh` が検知して exit 2)

### opt-in 自動提案 (KIOKU_HOT_AUTO_PROMPT)

`KIOKU_HOT_AUTO_PROMPT=1` を user が明示的に設定した場合のみ、Stop hook が
「hot.md を更新すべきか」という提案を additionalContext に流します。

- **default (未設定)**: 提案は出ません (opt-out 既定)
- **opt-in**: `export KIOKU_HOT_AUTO_PROMPT=1` を `~/.zshrc` 等に追加

理由 (26042304 meeting §3.1): hot.md は Git sync 対象で境界が session-logs より
厳しいため、自動化は user の明示的な承諾のみを信頼する (fail-safe 設計)。
