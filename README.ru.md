...
1. Создайте новый Vault в Obsidian (например, `~/kioku-vault/main`)
2. Создайте приватный репозиторий на GitHub (например, `kioku-vault`)
...
# Откройте ~/kioku-vault/main/ как Vault в Obsidian
# Повторите шаги 2–6
```
...
|---|---|---|
| `OBSIDIAN_VAULT` | нет (обязательна) | Корень Vault. auto-ingest/lint используют запасной путь `${HOME}/kioku-vault/main` |
...
- **Phase 1 (Web UI shell + Bases dashboard)**: `kioku_generate_viz mode=shell` для 8-tab self-contained HTML, Bases dashboard renderer (`.base` парсится только Node stdlib), Path C+β design boundary (без editor, без external CDN, без fetch)
- **Phase 2 (Claude-augmented Search)**: `kioku_search` с `intent` param + discoverQueries 3→7 sources, shell Search tab с Tier 1 offline filter + Tier 2 "Углубленный поиск с Claude" deep-link, attack surface 0 (без HTTP server)
...
- **Phase 4 (Sync polish)**: `sync-vault.mjs` Git push retry queue + exponential backoff + persistent resume, `classifyGitError` (5 категорий), `maskCredentials` (token/SSH redact), `doctor.sh` sync state diagnostic
- **Hardened design contract**: offline-first / textContent only / 0 external deps / 0 added attack surface — выдержано на всех 4 phases
- **Tests**: 44 новых BLUE-* (Phase 4 cumulative, retry / classify / mask / doctor diagnostic + sync-diagnostic.test.sh)
- **subagent-driven N=28 of 28 cycle** — Sprint 4 全 cycle завершен, formula LEARN#7 5-layer × 4 phases
...
- **`kioku_health` (11-й MCP tool)** — Возвращает 6 core метрик в JSON: `orphan` / `stale (>30d)` / `duplicate title` / `hot.md age` / `last ingest` / `unprocessed session-logs`. Каждая метрика с конкретным `next_actions`. Read-only.
- **`scripts/generate-health.mjs`** — `bash scripts/generate-health.mjs` пишет `wiki/meta/health.md`. Dashboard view "Wiki Health" добавлен в `templates/wiki/meta/dashboard.base`.
- **Sprint 2 stretch (запланировано для v0.7.5)** — broken wikilink / source_sha256 duplicate / и т.д.
- Tests: **23 BLUE-HEALTH-* + MCP-HEALTH-* + 9 drift (11 tools) + 475+ Node + 22 Bash все green**
...
- **Hook PostCompact** — `install-hooks.sh` теперь подключает 6-е событие (`PostCompact`), которое повторно внедряет только hot.md (index.md уже в контексте после компактификации, поэтому пропускается для избежания раздувания токенов)
- **Opt-in Stop prompt** (`KIOKU_HOT_AUTO_PROMPT=1`) — При явной установке конец сессии вызывает предложение обновить hot.md. **По умолчанию OFF** — hot.md синхронизируется через Git и имеет более строгую границу безопасности, чем session-logs, поэтому автоматический prompt требует явного согласия пользователя
- **Граница безопасности сохранена** — hot.md проходит через `applyMasks()` (маскирование паттернов API key / token) перед инъекцией, включён в цель обхода scan-secrets.sh, отклоняет symlink escape через `realpath` (пути вне vault отклонены) и обрезается на 4000 символах с WARN log
- **Согласование схемы hook Claude Code v2 (4 hotfix)** — Claude Code v2 использует разные схемы вывода для разных событий: `hookSpecificOutput` поддерживается только для `PreToolUse` / `UserPromptSubmit` / `PostToolUse`; `PostCompact` и `Stop` должны использовать `systemMessage` верхнего уровня. Устаревший v1 flat `{additionalContext}` молча отбрасывается в v2. Hotfix 1-4 мигрируют весь вывод hook в правильную схему для каждого события
- Тесты: **47 Node-утверждений** (HOT-1..9d + HOT-V1/V2 + регрессия session-logger + H1-H5 injector) **+ 488 Bash-утверждений** (IH-PC1/2 + SS-H1 + cron-guard-parity CGP-2 + 15 существующих suites), все зелёные
- [Release v0.5.1](https://github.com/megaphone-tokyo/kioku/releases/tag/v0.5.1) — `kioku-wiki-0.5.1.mcpb` прикреплён (9.2 МБ)
...
- Локальный stdio MCP-сервер (`mcp/`), предоставляющий шесть инструментов — `kioku_search`, `kioku_read`, `kioku_list`, `kioku_write_note`, `kioku_write_wiki`, `kioku_delete`
