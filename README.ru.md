...
7. **Инжест внешних источников (PDF / URL)**: `kioku_ingest_pdf` извлекает и резюмирует локальные PDF-файлы, размещённые в `raw-sources/`; `kioku_ingest_url` загружает HTTP(S) статьи через Mozilla Readability, сохраняет Markdown + изображения в `raw-sources/<dir>/fetched/` и автоматически перенаправляет PDF-ссылки в PDF-пайплайн. Большие PDF (≥ 2 чанков) используют отсоединённый (detached) процесс суммаризации, возвращая результат за ≤ 5 с (безопасно для 60-секундного таймаута Claude Desktop)
8. **Навыки Wiki Ingest**: Слеш-команды `/wiki-ingest-all` и `/wiki-ingest` импортируют существующие знания проекта в Wiki
9. **Изоляция секретов**: `session-logs/` остаётся локальным на каждой машине (`.gitignore`). Только `wiki/` / `raw-sources/` / `templates/` / `CLAUDE.md` управляются через Git
