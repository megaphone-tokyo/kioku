...
1. Obsidian で新規 Vault を作成（例: `~/kioku-vault/main`）
2. GitHub で Private リポジトリを作成（例: `kioku-vault`）
...
# Obsidian で ~/kioku-vault/main/ を Vault として開く
# 上記の 2〜6 の手順を繰り返す
```
...
| 変数 | 既定値 | 役割 |
|---|---|---|
| `OBSIDIAN_VAULT` | なし (必須) | Vault ルート。auto-ingest/lint のみ `${HOME}/kioku-vault/main` をデフォルトとしてフォールバック |
