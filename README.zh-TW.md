...
1. 在 Obsidian 中建立新的 Vault（例如 `~/kioku-vault/main`）
2. 在 GitHub 上建立 Private 儲存庫（例如 `kioku-vault`）
...
|---|---|---|
| `OBSIDIAN_VAULT` | 無（必填） | Vault 根目錄。auto-ingest/lint 會回退至 `${HOME}/kioku-vault/main` |
...
- **Phase 1 (Web UI shell + Bases dashboard)**：`kioku_generate_viz mode=shell` 輸出 8-tab self-contained HTML，Bases dashboard renderer（僅用 Node stdlib 解析 `.base`），Path C+β design boundary（無 editor、無 external CDN、無 fetch）
- **Phase 2 (Claude-augmented Search)**：`kioku_search` 加入 `intent` param + discoverQueries 3→7 sources，shell Search tab 含 Tier 1 offline filter + Tier 2「用 Claude 深入搜尋」deep-link，attack surface 0（無 HTTP server）
- **Phase 3 (Mobile responsive)**：shell + viz responsive CSS + 漢堡選單 + tap target 44pt+，Tier 2 Mobile deep-link `claude://` URI + 1.5s timeout + claude.ai fallback，Mobile simplified view（textContent only）+ 300KB performance budget
...
- **Hardened design contract**：offline-first / textContent only / 0 external deps / 0 added attack surface — 4 phase 一以貫之
- **Tests**：44 個新增 BLUE-*（Phase 4 累計，retry / classify / mask / doctor diagnostic + sync-diagnostic.test.sh）
...
- **`kioku_health` (第 11 個 MCP tool)** — 返回 6 個 core metrics 作為 JSON：`orphan` / `stale (>30d)` / `duplicate title` / `hot.md age` / `last ingest` / `unprocessed session-logs`。每個 metric 配有具體的 `next_actions`。Read-only。
- **`scripts/generate-health.mjs`** — `bash scripts/generate-health.mjs` 寫入 `wiki/meta/health.md`。Dashboard view "Wiki Health" 加入到 `templates/wiki/meta/dashboard.base`。
- **Sprint 2 stretch (計劃在 v0.7.5)** — broken wikilink / source_sha256 duplicate / 等
...
- **PostCompact hook** — `install-hooks.sh` 現在接入第 6 個 event（`PostCompact`）,僅重新注入 hot.md（壓縮後 index.md 已在 context 中,因此跳過以避免 token 膨脹）
- **Opt-in Stop prompt**（`KIOKU_HOT_AUTO_PROMPT=1`）— 明確設定時,session 結束會觸發 hot.md 更新建議 prompt。**Default OFF** — hot.md 透過 Git 同步,比 session-logs 有更嚴格的 security boundary,因此自動 prompt 需要使用者明確同意
- **安全邊界維持** — hot.md 在注入前通過 `applyMasks()`（API key / token 模式遮罩）,包含在 scan-secrets.sh walk 目標中,透過 `realpath` 拒絕 symlink escape（vault 外路徑被拒絕）,並在 4000 字元處截斷 + 輸出 WARN log
- **Claude Code v2 hook schema 對齊（4 個 hotfix）** — Claude Code v2 對不同 event 使用不同的 output schema：`hookSpecificOutput` 僅支援 `PreToolUse` / `UserPromptSubmit` / `PostToolUse`；`PostCompact` 和 `Stop` 必須使用頂層 `systemMessage`。舊的 v1 扁平 `{additionalContext}` 在 v2 中被靜默丟棄。Hotfix 1-4 將所有 hook 輸出按 event 遷移到正確的 schema
- 測試：**Node 47 assertions**（HOT-1..9d + HOT-V1/V2 + session-logger regression + injector H1-H5）**+ Bash 488 assertions**（IH-PC1/2 + SS-H1 + cron-guard-parity CGP-2 + 現有 15 suites）,全綠
...
- 本機 stdio MCP 伺服器（`mcp/`）提供六項工具 — `kioku_search`、`kioku_read`、`kioku_list`、`kioku_write_note`、`kioku_write_wiki`、`kioku_delete`
